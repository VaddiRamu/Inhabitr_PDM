...
getQuotes1(companyName?,projectName?,start?,count?):Observable<any>{
  return this.http.get<any>(environment.endPoint + 'load/customer/quotes?user_id='+this.ls.getFromLocal().userId+ '&company_name='+companyName+'&project_name='+projectName+'&start='+start+'&count='+count);
...
getQuotationDetails(quoteid){
  return this.http.get<any>(environment.endPoint + 'quote/items?quote_id='+quoteid);
