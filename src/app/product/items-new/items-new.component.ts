...
export class ItemsNewComponent implements OnInit {
  ...
  publisheValue = [{ name: "Published to Inhabitr.com", value: "register_with_ops", type: "Publish Status", isPublishOps: 1, isUnpublishOps: null },
  { name: "Not Published to Inhabitr.com", value: "unregister_from_ops", type: "Publish Status", isPublishOps: null, isUnpublishOps: 1 },
  ...
  productList() {
    ...
    this.spinner.show();
    this.isLoading = true;
    this.shop.getItems(this.page, this.parameters, this.publishVal).subscribe(
      (res: any) => {
        //console.log(res)
        this.spinner.hide();
  ...
  invQty(data:any,id:any){
    ...
    if(this.resultData){
      if(!this.warehouse){
        data.forEach(element=>{
          if(element?.is_inhabitr_warehouse=='Y'){
            invTotal += parseInt(element.non_assigned_inv); 
          }
        });
      }
      ...
      else if(this.inputData.warehouse){
        data.forEach(element=>{
          if(element?.is_inhabitr_warehouse=='Y' && element.warehouse_id==id){
            invTotal = element?.non_assigned_inv;
          }
  ...
  // Inhabitr Inv Qty
  // Supplier Inv Qty
  suppQty(data:any,id:any){
    let supTotal: number=0;
    if(this.resultData){
      if(!this.warehouse){
        data.forEach(element=>{
          if(element?.is_inhabitr_warehouse=='N'){
            supTotal += parseInt(element.supplier_quantity)
          }
        });
      }
      else if(this.warehouse){
  ...
  // checkIsCitySelect() {
  //   let filters = [];
  //   filters = JSON.parse(localStorage.getItem("filter"))? JSON.parse(localStorage.getItem("filter")): filters;
  //   let index = filters.findIndex((x) => x.type === "City");
  //   if (index != -1 && filters[index].selected.length) {
  //     this.modalVal = "warehouseContent";
  //   } else {
  //     this.modalVal = "Warehouse";
  //     this.deliverySupplierWarehouseList("delivery");
  //     this.deliverySupplierWarehouseList("supplier");
  //   }
  // }
  // checkIsWareHouseSelect() {
  //   let filters = [];
  //   filters = JSON.parse(localStorage.getItem("filter"))? JSON.parse(localStorage.getItem("filter")): filters;
  //   let index = filters.findIndex((x) => x.type === "Warehouse");
  //   if (index != -1 && filters[index].selected.length) {
  //     this.modalVal = "cityContent";
  //   } else {
  //     this.modalVal = "City";
    ...
    if (data === "delivery" && !this.s_id) {
      ...
      this.shop.getSupplierPartner().subscribe((res) => {
        if (res) {
          ...
          console.log(this.countOfCategorys); 
        }
      });
    }
    else if (data === "supplier" && this.s_id) {
  ...
  getSuppliersList() {
    ...
    wareSupplierList.forEach(ele => {
      ...
      // else if(ele.type=='Warehouse' && this.w_id || ele?.type=='Warehouse' && this.s_id){
      //   this.shop.getSupplierWarehouseFilter(this.s_id, this.w_id, this.c_id).subscribe((res) => {
      //     this.loadingData=false;
  ...
  filterResult() {
    ...
    this.filterData?.forEach((ele) => {
      ...
      if (ele.type === "City") {
        ...
        console.log(this.supplierIsTrue);
        // this.deliverySupplierWarehouseList('delivery');
        this.getSuppliersList();
